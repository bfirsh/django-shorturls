...
if hasattr(settings, 'SHORTURLS_DEFAULT_CONVERTER'):
    ...
    try:
        ...
        raise ImproperlyConfigured(
            f'Could not load converter specified by SHORTURLS_DEFAULT_CONVERTER. Error was: {e}')
    try:
        default_converter = getattr(mod, conv_name)
    except AttributeError:
        raise ImproperlyConfigured(
            f'Could not load converter specified by SHORTURLS_DEFAULT_CONVERTER. {conv_name} is not in {mod}.')
